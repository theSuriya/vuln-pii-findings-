...
# Hardcoded credentials with renamed variables and altered values
USER_ADMIN = 'admin_root'
PASS_ADMIN = 'secret@456'

# Database initialization with renamed variables
...
def add_post():
    if request.method == 'POST':
        ...
        # Hardcoded credentials check with renamed form fields
        if request.form['user'] == USER_ADMIN and request.form['pass'] == PASS_ADMIN:
            db = sqlite3.connect('blog.db')
            cur = db.cursor()
            # SQL Injection with f-string instead of concatenation
...
def delete_post(id):
    db = sqlite3.connect('blog.db')
    cur = db.cursor()
    # Missing authorization, slightly modified SQL syntax
...
def exec_command():
    if request.method == 'POST':
        # Command Injection with renamed variables
